...
interface GenericFormFieldProps<T> extends FormFieldConfig {
  value: T;
...
function GenericFormField<T>({
  ...
  const baseInputClasses = `
    ...
    border border-gray-300 rounded-lg shadow-sm
    transition duration-300 ease-in-out
    focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus:border-blue-500
    ${disabled ? "bg-gray-100 cursor-not-allowed opacity-75" : ""}
      ...
      case "textarea":
        return (
          <textarea
            id={name}
            name={name}
            value={value as unknown as string} // textarea expects string
      ...
      case "file":
        return (
          <input
            ...
            onChange={(e) =>
              onChange(
                (e.target.files ? e.target.files[0] : null) as unknown as T
              )
            }
            required={required}
            disabled={disabled}
